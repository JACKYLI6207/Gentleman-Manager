...
- 快照掃描：支援多分類掃描、分類獨立存檔、可調整掃描節奏、失敗頁可稍後補掃與進度顯示。
- 接續/更新快照：未完成快照可接續建立或依 ID 增量更新；已完成的快照可在有新內容時合併更新。
...
- `接續掃描` 依快照完成度分流：未完成可接續建立，已完成則以增量方式檢查是否有新項目。
- 增量更新會從列表前段比對已知項目，遇見足夠多已有資料後停止；若沒有新項目會提示已是最新。
- 分類快照完成或更新後會自動排序校準並輸出至 `Website Snapshot`；同分類舊檔自動移入 `Website Snapshot/old`（含更新掃描無新增時的同步導出）。
