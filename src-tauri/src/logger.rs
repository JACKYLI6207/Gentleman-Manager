    ...
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        match serde_json::from_str::<LogEvent>(&log_string) {
            ...
            Err(err) => {
                ...
                tracing::error!(log_string, err_msg, "將日誌字串解析為LogEvent失敗");
            }
        }
        Ok(buf.len())
    }
...
pub fn init(app: &AppHandle) -> anyhow::Result<()> {
    ...
    let lib_target = lib_module_path.split("::").next().context(format!(
        "解析lib_target失敗: lib_module_path={lib_module_path}"
    ))?;
    // 過濾掉來自其他庫的日誌
    ...
    let log_event_layer = layer()
        ...
        .with_line_number(true)
        .json()
        // 過濾掉來自這個文件的日誌(LogEvent解析失敗的日誌)，避免無限遞歸
    ...
    RELOAD_FN.get_or_init(move || {
        ...
        Box::new(move || {
            let (file_layer, guard) = create_file_layer(&app)?;
            reload_handle.reload(file_layer).context("reload失敗")?;
    ...
    S: Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    // 如果不啟用檔案日誌，則返回一個佔位用的sink layer，不創建也不輸出日誌檔案
    ...
    let file_appender = RollingFileAppender::builder()
        ...
        .build(&logs_dir)
        .context("創建RollingFileAppender失敗")?;
...
async fn file_log_watcher(app: AppHandle) {
    ...
    let event_handler = move |res| {
        tauri::async_runtime::block_on(async {
            if let Err(err) = sender.send(res).await.map_err(anyhow::Error::from) {
                let err_title = "發送日誌檔案watcher事件失敗";
    ...
    let logs_dir = match logs_dir(&app) {
        ...
        Err(err) => {
            let err_title = "日誌檔案watcher獲取日誌目錄失敗";
            let string_chain = err.to_string_chain();
            tracing::error!(err_title, message = string_chain);
            return;
        }
    ...
    if let Err(err) = watcher
        ...
        .map_err(anyhow::Error::from)
    {
        let err_title = "日誌檔案watcher監聽日誌目錄失敗";
        let string_chain = err.to_string_chain();
        tracing::error!(err_title, message = string_chain);
        return;
    }
    ...
    while let Some(res) = receiver.recv().await {
        match res.map_err(anyhow::Error::from) {
            ...
            Err(err) => {
                let err_title = "接收日誌檔案watcher事件失敗";
                let string_chain = err.to_string_chain();
                tracing::error!(err_title, message = string_chain);
            }
        }
